...
void UBaseCharacterMovementComponent::BeginPlay()
{
	...
	GCCharacter = Cast<ABaseCharacter>(CharacterOwner);
...
bool UBaseCharacterMovementComponent::TryStartSprint()
{
...
void UBaseCharacterMovementComponent::SetIsOutOfStamina(bool bNewOutOfStamina)
{
...
void UBaseCharacterMovementComponent::RequestStandUp()
{
	bWantsToProne = false;
	bWantsToCrouch = false;
}

void UBaseCharacterMovementComponent::RequestCrouch()
{
	bWantsToProne = false;
	bWantsToCrouch = true;
}
...
bool UBaseCharacterMovementComponent::TryCrouchOrProne(float NewCapsuleHalfHeight, float NewCapsuleRadius, float& ScaledHalfHeightAdjust)
{
...
void UBaseCharacterMovementComponent::FillWakeUpParams(FWakeUpParams& WakeUpParams) const
{
...
bool UBaseCharacterMovementComponent::TryWakeUpToState(EPosture DesiredPosture, bool bClientSimulation)
{
...
bool UBaseCharacterMovementComponent::TryWakeUp(float DesiredUnscaledHalfHeight, const FWakeUpParams& WakeUpParams, float& ScaledHalfHeightAdjust,
...
bool UBaseCharacterMovementComponent::CanProne()
{
	return (IsMovingOnGround() || IsSliding()) && !UpdatedComponent->IsSimulatingPhysics(); 
}

void UBaseCharacterMovementComponent::RequestProne()
{
	bWantsToProne = true;
	bWantsToCrouch = false;
}
...
bool UBaseCharacterMovementComponent::TryStartMantle(const FMantlingMovementParameters& NewMantlingParameters)
{
	...
	GetWorld()->GetTimerManager().SetTimer(MantlingTimerHandle, this,
    	&UBaseCharacterMovementComponent::EndMantle, MantlingParameters.Duration);
...
void UBaseCharacterMovementComponent::PhysCustomMantling(float DeltaTime, int32 Iterations)
{
...
float UBaseCharacterMovementComponent::GetActorToClimbableProjection(const ALadder* Ladder,
...
bool UBaseCharacterMovementComponent::TryStartClimbing(const ALadder* Ladder)
{
...
void UBaseCharacterMovementComponent::StopClimbing(EStopClimbingMethod StopClimbingMethod)
{
...
float UBaseCharacterMovementComponent::GetClimbingSpeedRatio() const
{
...
void UBaseCharacterMovementComponent::PhysCustomClimbing(float DeltaTime, int32 Iterations)
{
...
bool UBaseCharacterMovementComponent::TryStartZiplining(const FZiplineParams& NewZiplineParams)
{
...
void UBaseCharacterMovementComponent::StopZiplining()
{
	SetMovementMode(MOVE_Falling);
	GCCharacter->OnActionEnded(ECharacterAction::Zipline);
}

bool UBaseCharacterMovementComponent::IsZiplining() const
{
	return IsInCustomMovementMode(EGCMovementMode::CMOVE_Zipline);
}

void UBaseCharacterMovementComponent::PhysCustomZiplining(float DeltaTime, int32 Iterations)
{
...
void UBaseCharacterMovementComponent::OnPlayerCapsuleHit(UPrimitiveComponent* HitComponent, AActor* OtherActor,
...
ESide UBaseCharacterMovementComponent::GetWallrunSideFromNormal(const FVector& Normal) const
{
...
FVector UBaseCharacterMovementComponent::GetWallrunSurfaceNormal(const ESide& Side, const FVector& CharacterLocationDelta) const
{
...
void UBaseCharacterMovementComponent::RequestWallrunning()
{
	WallrunData.bWantsToWallrun = true;
}

void UBaseCharacterMovementComponent::StopWallrunning(bool bResetTimer)
{
...
void UBaseCharacterMovementComponent::JumpOffWall()
{
...
void UBaseCharacterMovementComponent::PhysCustomWallRun(float DeltaTime, int32 iterations)
{
...
bool UBaseCharacterMovementComponent::TryStartSliding()
{
	...
	GetWorld()->GetTimerManager().SetTimer(SlideData.TimerHandle, this,
		&UBaseCharacterMovementComponent::StopSliding, SlideSettings.Duration);
...
void UBaseCharacterMovementComponent::StopSliding()
{
	if (!IsSliding())
	{
...
void UBaseCharacterMovementComponent::ResetFromSliding()
{
	...
	GetWorld()->GetTimerManager().SetTimer(SlideData.CooldownTimerHandle, this,
		&UBaseCharacterMovementComponent::ResetSlide, SlideSettings.CooldownTime);
}

void UBaseCharacterMovementComponent::ResetSlide()
{
	SlideData.bCanSlide = true;
}

void UBaseCharacterMovementComponent::PhysCustomSliding(float DeltaTime, int32 Iterations)
{
	const float G = -GetGravityZ();
...
void UBaseCharacterMovementComponent::InitPostureHalfHeights()
{
...
bool UBaseCharacterMovementComponent::IsInCustomMovementMode(const EGCMovementMode& Mode) const
{
	return MovementMode == MOVE_Custom && CustomMovementMode == (uint8)Mode;
}

UDebugSubsystem* UBaseCharacterMovementComponent::GetDebugSubsystem() const
{
...
EMovementMode UBaseCharacterMovementComponent::GetMovementMode()
{
